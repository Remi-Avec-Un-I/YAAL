...
use std::{fs, path::{Path, PathBuf}, os::raw::c_char, ffi::CStr};
            ...
            Ok(lib) => {
                match (
                    ...
                    (Ok(info_ptr), Ok(get_entries), Ok(handle_selection)) => {
                        ...
                        println!("  Version: {}", CStr::from_ptr(info.version).to_string_lossy());
                        println!("  Description: {}", CStr::from_ptr(info.description).to_string_lossy());
                        ...
                        for i in 0..entries.length {
                            ...
                            println!("  Description: {}", CStr::from_ptr(entry.description).to_string_lossy());
